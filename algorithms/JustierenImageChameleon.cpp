                                            ...
                                            float dynamic_R)
{
	...
	if (m_test == false ? cell_x >= CELLS_NUM_X : cell_x > CELLS_NUM_X) {
		PRINT_ERR(true, "x >= %lu", static_cast<unsigned long>(CELLS_NUM_X));
		return -1;
	}
	if (m_test == false ? cell_y >= CELLS_NUM_Y : cell_y > CELLS_NUM_Y) {
		PRINT_ERR(true, "y >= %lu", static_cast<unsigned long>(CELLS_NUM_Y));
		return -1;
	}
	...
	if (   m_Top_Left.x < 0 || m_Top_Left.y < 0
		...
		PRINT_ERR(true, "one component of the edge nodes less than 0");
		return -1;
	}
	...
	if (angle_yaw >= ANGLE_MAX || angle_yaw < 0) {
		PRINT_ERR(true, "angle_yaw - something went wrong");
		return -1;
	}
	...
	if (angle_pitch >= ANGLE_MAX || angle_pitch < 0) {
		PRINT_ERR(true, "angle_pitch - something went wrong");
		return -1;
	}
                                            ...
                                            float dynamic_R)
{
	if (angle_yaw == nullptr || angle_pitch == nullptr) {
		PRINT_ERR(true, "angle_yaw == nullptr or angle_pitch == nullptr");
		return -1;
	}
                                           ...
                                           float dynamic_R)
{
	...
	if (   angle_yaw < std::min(m_Top_Left.x, m_Bottom_Left.x)
		...
		PRINT_ERR(true, "angle_yaw < minLeft_x || angle_yaw > maxRight_x");
		return -1;
	}
	...
	if (   angle_pitch < std::min(m_Top_Left.y, m_Top_Right.y)
		...
		PRINT_ERR(true, "angle_pitch < minTop_y || angle_pitch > maxBottom_y");
		return -1;
	}
	...
	if (   (m_test == false ? cell_x_tmp >= CELLS_NUM_X : cell_x_tmp > CELLS_NUM_X)
	    || cell_x_tmp < 0) {
		PRINT_ERR(true, "cell_x >= CELLS_NUM_X || cell_x < 0");
		return -1;
	}
	...
	if (   (m_test == false ? cell_y_tmp >= CELLS_NUM_Y : cell_y_tmp > CELLS_NUM_Y)
	    || cell_y_tmp < 0) {
		PRINT_ERR(true, "cell_y >= CELLS_NUM_Y || cell_y < 0");
		return -1;
	}
                                           ...
                                           float dynamic_R)
{
	if (cell_x == nullptr || cell_y == nullptr) {
		PRINT_ERR(true, "cell_x == nullptr or cell_y == nullptr");
		return -1;
	}
                                             ...
                                             Point_f& point)
{
	...
	if (angle_x < 0 || angle_x >= ANGLE_MAX) {
		PRINT_ERR(true, "x < 0 or x >= %lu", static_cast<unsigned long>(ANGLE_MAX));
		return -1;
	}
	if (angle_y < 0 || angle_y >= ANGLE_MAX) {
		PRINT_ERR(true, "y < 0 or y >= %lu", static_cast<unsigned long>(ANGLE_MAX));
		return -1;
	}
                                                 ...
                                                 const uint32_t j)
{
	...
	if (obj.calcCells(p_x[i].Top_Left, p_y[j].Top_Left, cell_x, cell_y) != 0) {
		PRINT_ERR(true, "i = %d, j = %d, calcCells(): Top_Left",
		          static_cast<int>(i), static_cast<int>(j));
		return -1;
	}
	if (cell_x != 0 || cell_y != 0) {
		PRINT_ERR(true, "i = %d, j = %d, check cells: Top_Left",
		          static_cast<int>(i), static_cast<int>(j));
		return -1;
	}

	if (obj.calcCells(p_x[i].Top_Right, p_y[j].Top_Right, cell_x, cell_y) != 0) {
		PRINT_ERR(true, "i = %d, j = %d, calcCells(): Top_Right",
		          static_cast<int>(i), static_cast<int>(j));
		return -1;
	}
	if (cell_x != 320 || cell_y != 0) {
		PRINT_ERR(true, "i = %d, j = %d, check cells: Top_Right",
		          static_cast<int>(i), static_cast<int>(j));
		return -1;
	}

	if (obj.calcCells(p_x[i].Bottom_Left, p_y[j].Bottom_Left, cell_x, cell_y) != 0) {
		PRINT_ERR(true, "i = %d, j = %d, calcCells(): Bottom_Left",
		          static_cast<int>(i), static_cast<int>(j));
		return -1;
	}
	if (cell_x != 0 || cell_y != 320) {
		PRINT_ERR(true, "i = %d, j = %d, check cells: Bottom_Left",
		          static_cast<int>(i), static_cast<int>(j));
		return -1;
	}

	if (obj.calcCells(p_x[i].Bottom_Right, p_y[j].Bottom_Right, cell_x, cell_y) != 0) {
		PRINT_ERR(true, "i = %d, j = %d, calcCells(): Bottom_Right",
		          static_cast<int>(i), static_cast<int>(j));
		return -1;
	}
	if (cell_x != 320 || cell_y != 320) {
		PRINT_ERR(true, "i = %d, j = %d, check cells: Bottom_Right",
		          static_cast<int>(i), static_cast<int>(j));
		return -1;
	}
