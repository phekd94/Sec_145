...
int32_t NeuralNetwork::readParameters(const QString& pathToModel)
{
	...
	{
		PRINT_ERR("Can't read a m_num_conv_layers from file %s",
	...
	for (uint32_t i = 0; i < m_num_conv_layers; ++i)
	{
		...
		{
			PRINT_ERR("Can't read m_in_conv_size from file %s",
			          fileName.toStdString().c_str());
			return -1;
		}
	...
	if (readVarFromFile(f, var) != 0)
	{
		PRINT_ERR("Can't read a m_out_pooling_size from file %s",
		          fileName.toStdString().c_str());
		return -1;
	}
	...
	for (uint32_t i = 0; i < m_num_conv_layers; ++i)
	{
		...
		{
			PRINT_ERR("Can't read m_kernels_names from file %s",
			          fileName.toStdString().c_str());
			return -1;
		}
	...
	{
		PRINT_ERR("Can't read a m_num_dense_layers from file %s",
		          fileName.toStdString().c_str());
		return -1;
	}
