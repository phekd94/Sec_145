...
int32_t readVarFromFile(QFile& file, uint32_t& var)
{
	...
	if (ar.isEmpty() == true)
	{
		PRINT_ERR("Bad data (empty)");
		return -1;
	}
	ar.resize(ar.size() - 1); // exclude /n
	bool ok;
	var = static_cast<uint32_t>(ar.toUInt(&ok));
	if (false == ok)
	{
		PRINT_ERR("Bad data (ok)");
		return -1;
	}
	return 0;
}
...
int32_t readVarFromFile(QFile& file, double& var)
{
	...
	ar.resize(ar.size() - 2); // exclude /r/n
	bool ok;
	var = ar.toDouble(&ok);
	if (false == ok)
	{
...
int32_t readVarFromFile(QFile& file, QString& var)
{
	...
	if (var.isEmpty() == true)
	{
		PRINT_ERR("Bad data (empty var)");
		return -1;
	}

	// Remove "\r\n" from result string
	//var.remove(var.size() - 2, 2);
	var.remove(var.size() - 1, 1);
...
int32_t compareFiles(QFile& file_1, QFile& file_2, const uint32_t num, bool& res)
{
	...
	{
		PRINT_DBG(true, "Files have extra data");
	}
