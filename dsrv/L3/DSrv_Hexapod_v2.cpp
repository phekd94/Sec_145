...
//-------------------------------------------------------------------------------------------------
const std::vector<uint32_t>& DSrv_Hexapod_v2::getDataSize() const
{
	return m_data_size;
}

//-------------------------------------------------------------------------------------------------
int32_t DSrv_Hexapod_v2::start()
{
	...
	connect(getSerialPort(), &QSerialPort::errorOccurred, this, &DSrv_Hexapod_v2::onErrorOccured);

	return 0;
}

//-------------------------------------------------------------------------------------------------
int32_t DSrv_Hexapod_v2::stop()
{
...
//-------------------------------------------------------------------------------------------------
int32_t DSrv_Hexapod_v2::dataParser(uint8_t* data, uint32_t size)
{
	...
	while (size > 0) {
		if (0 == m_pktRemSize) {
			...
			// Data
			if (setData(data, 1, add) != 0) {
				PRINT_ERR(true, PREF, "setData()");
				return -1;
			}
			--m_pktRemSize;
			if (0 == m_pktRemSize) {
				if (completeData() != 0) {
					PRINT_ERR(true, PREF, "completeData() != 0");
					return -1;
				}
				...
				uint8_t* data_res;
				uint32_t size_res;
				if (getData(&data_res, &size_res) != 0) {
					PRINT_ERR(true, PREF, "getData() != 0");
					return -1;
				}
				for (uint32_t i = m_data_size[m_cmd_id];
				     i < sizeof(uint32_t);
				     ++i) {
					data_res[i] = 0;
				}
...
//-------------------------------------------------------------------------------------------------
void DSrv_Hexapod_v2::onErrorOccured(QSerialPort::SerialPortError err)
{
	switch (err) {
...
//-------------------------------------------------------------------------------------------------
void DSrv_Hexapod_v2::onSendCommand(const uint32_t cmd, const uint32_t* data, const uint32_t size)
{
	switch (cmd) {
	...
	case HEXAPOD_CMD_SEND:
		if (sendData(reinterpret_cast<const uint8_t*>(data), size, nullptr, 0) != 0) {
	...
	default:
		PRINT_ERR(true, PREF, "Unknown a command value from Widget");
		break;
	}
}
...
//-------------------------------------------------------------------------------------------------
int32_t DSrv_Hexapod_v2_test::logic(DSrv_Hexapod_v2& obj, DSrv_Hexapod_v2_test& obj_test)
{
	if (obj.start() == 0) {
